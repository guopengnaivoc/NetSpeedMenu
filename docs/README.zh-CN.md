...
“网速”是一款简洁的 macOS 菜单栏工具。菜单栏上方的 `↑` 显示实时上传速度，下方的 `↓` 显示实时下载速度。显示区域固定为 42 点宽，不占用 Dock，也不会弹出多余通知。
...
从本仓库的 [Releases](../../../releases/latest) 页面下载 `NetSpeedMenu-1.5-universal.dmg`。DMG 是推荐安装方式。
...
1. 双击 `NetSpeedMenu-1.5-universal.dmg`。
2. 将“网速.app”拖入旁边的“Applications”文件夹。
3. 打开“应用程序”文件夹，找到“网速”。
4. 第一次启动请按照下面的“首次打开”步骤操作。

也可以使用 PKG：右键或按住 Control 点击 `NetSpeedMenu-1.5-universal.pkg`，选择“打开”，然后按照系统安装器提示操作。安装器可能要求管理员密码。
