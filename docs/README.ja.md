...
# NetSpeedMenu 1.5 取扱説明書

[简体中文](README.zh-CN.md) · [English](README.en.md) · [Français](README.fr.md) · [ホーム](../README.md)

## 概要

NetSpeedMenu（网速）は、macOS のメニューバーに通信速度を表示する軽量アプリです。上段の `↑` はアップロード速度、下段の `↓` はダウンロード速度を示します。表示領域は 42 ポイント固定で、Dock には表示されません。
...
このリポジトリの [Releases](../../../releases/latest) ページから `NetSpeedMenu-1.5-universal.dmg` をダウンロードしてください。DMG 版を推奨します。
...
1. `NetSpeedMenu-1.5-universal.dmg` をダブルクリックします。
2. `网速.app` を隣の Applications フォルダへドラッグします。
3. 「アプリケーション」フォルダで `网速` を探します。
4. 下記の初回起動手順に従ってください。

PKG 版も利用できます。`NetSpeedMenu-1.5-universal.pkg` を Control キーを押しながらクリックし、「開く」を選択してインストーラの指示に従います。管理者パスワードが必要になる場合があります。
