...
このリポジトリの [Releases](../../../releases/latest) ページから `NetSpeedMenu-1.3-universal.dmg` をダウンロードしてください。DMG 版を推奨します。
...
1. `NetSpeedMenu-1.3-universal.dmg` をダブルクリックします。
2. `网速.app` を隣の Applications フォルダへドラッグします。
3. 「アプリケーション」フォルダで `网速` を探します。
4. 下記の初回起動手順に従ってください。

PKG 版も利用できます。`NetSpeedMenu-1.3-universal.pkg` を Control キーを押しながらクリックし、「開く」を選択してインストーラの指示に従います。管理者パスワードが必要になる場合があります。
